...
export default function SignUp({ navigation, route }: SignUpProps) {
  ...
  async function saveUserInSupabase(user: any) {
    try {
      const { error } = await supabase.from('profiles').insert([
        { id: user.uid, full_name: fullName, email: user.email, role },
      ]);
  ...
  const signInWithGoogle = async () => {
    try {
      const CLIENT_ID = '838270482544-be5cl99sbjpajs8dpv4t3r3mo7tdd426.apps.googleusercontent.com'; // استبدل هذا بـ Client ID الخاص بك
      const REDIRECT_URI = AuthSession.makeRedirectUri({ useProxy: true });

      const authUrl = `https://accounts.google.com/o/oauth2/v2/auth?response_type=token&client_id=${CLIENT_ID}&redirect_uri=${encodeURIComponent(
        REDIRECT_URI
      )}&scope=profile%20email`;

      const result = (await AuthSession.startAsync({ authUrl })) as AuthSession.AuthSessionResult & {
        params: { access_token?: string; id_token?: string };
      };
      ...
      if (result.type === 'success' && result.params.id_token) {
        ...
        navigationBasedOnRole(user); // التوجيه بناءً على الدور
      } else {
